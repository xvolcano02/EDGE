...
class EnvironmentManagerBase:
    def __init__(self, envs, projection_f, config):
        ...
        self.config = config
        self.experience_injection_mask = None

    def set_experience_injection_mask(self, mask) -> None:
        if mask is None:
            self.experience_injection_mask = None
        else:
            self.experience_injection_mask = np.asarray(mask, dtype=bool)

    def _should_inject_experience(self, index: int, init: bool = False) -> bool:
        if self.experience_injection_mask is None:
            return True
        return bool(self.experience_injection_mask[index])
