...
class RetrievalMemory(BaseMemory):
    ...
    - General experiences (Claude-style concise principles)
    - Task-specific experiences for the detected task type
    ...
    def __init__(
        ...
        device: str = "cuda",
        experiences_json_path: str = None
    ):
        """
        ...
        Args:
            ...
            experiences_json_path: Optional path to Claude-style experiences JSON
        """
        import torch
        ...
        # Load Claude-style experiences if available
        self.experiences = None
        if experiences_json_path is None:
            # Try to find experiences file in same directory as memory JSON
            experiences_path = os.path.join(
                os.path.dirname(memory_json_path),
                'claude_style_experiences.json'
            )
        ...
        if experiences_json_path and os.path.exists(experiences_json_path):
            ...
            print(f"[RetrievalMemory] Loaded Claude-style experiences: "
                  f"{len(self.experiences.get('general_experiences', []))} general, "
                  f"{sum(len(v) for v in self.experiences.get('task_specific_experiences', {}).values())} task-specific, "
                  f"{len(self.experiences.get('common_mistakes', []))} mistakes")
        else:
            print("[RetrievalMemory] No Claude-style experiences found, using legacy trajectory-based retrieval")
    ...
    def retrieve(
        ...
        If Claude-style experiences are available, returns concise experiences.
        ...
        Returns:
            Dictionary containing:
                - general_experiences: List of general experiences/principles
                - task_specific_experiences: List of task-type specific experiences
                - mistakes_to_avoid: List of common mistakes
                - task_specific_examples: (optional) List of similar trajectories
        """
        # Use Claude-style experiences if available
        if self.experiences:
            return self._retrieve_with_experiences(task_description, max_tokens, include_examples)
    ...
    def _retrieve_with_experiences(
        self,
        task_description: str,
        max_tokens: int,
        include_examples: bool
    ) -> Dict[str, Any]:
        """Retrieve using Claude-style experiences (concise, actionable)."""
        task_type = self._detect_task_type(task_description)

        # Get general experiences (most important ones)
        general_experiences = self.experiences.get('general_experiences', [])[:6]

        # Get task-specific experiences
        task_experiences = self.experiences.get('task_specific_experiences', {}).get(task_type, [])

        # Get common mistakes
        common_mistakes = self.experiences.get('common_mistakes', [])[:5]

        result = {
            'general_experiences': general_experiences,
            'task_specific_experiences': task_experiences,
    ...
    def _categorize_and_budget(
        ...
        """
        Categorize memories into general experiences and task-specific examples,
        applying token budget constraints.
        """
        # Extract general planning patterns (unique patterns across memories)
        general_experiences = self._extract_general_patterns(success_results)
        ...
        selected_general = self._truncate_by_tokens(general_experiences, general_tokens)
        ...
        return {
            'general_experiences': selected_general,
    ...
    def format_for_prompt(self, retrieved_memories: Dict[str, Any]) -> str:
        ...
        # Check if using Claude-style experiences (has 'task_specific_experiences' key)
        if 'task_specific_experiences' in retrieved_memories:
    ...
    def _format_claude_style(self, retrieved_memories: Dict[str, Any]) -> str:
        """Format Claude-style experiences (concise, actionable)."""
        sections = []
        task_type = retrieved_memories.get('task_type', 'unknown')

        # Format general experiences
        general_experiences = retrieved_memories.get('general_experiences', [])
        ...
        # Format task-specific experiences
        task_experiences = retrieved_memories.get('task_specific_experiences', [])
        if task_experiences:
            task_name = task_type.replace('_', ' ').title()
            lines = [f"### {task_name} Experiences"]
            for experience in task_experiences:
                title = experience.get('title', '')
                principle = experience.get('principle', '')
                when = experience.get('when_to_apply', '')
                lines.append(f"- **{title}**: {principle}")
                if when:
    ...
    def _format_legacy_style(self, retrieved_memories: Dict[str, Any]) -> str:
        ...
        if retrieved_memories.get('general_experiences'):
            general_lines = ["### General Planning Patterns:"]
            for i, experience in enumerate(retrieved_memories['general_experiences'], 1):
                general_lines.append(
                    f"{i}. [Pattern]: {experience['pattern']}\n"
                    f"   - Example: {experience['example_task']}"
                )
