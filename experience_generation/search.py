"""
Generate Claude-style experiences for Search agent using o3 API.
...
4. Hierarchical - general experiences + query-type-specific experiences
...
Generate 8-12 GENERAL EXPERIENCES that apply across ALL question types. These should be:
...
def generate_query_type_experiences(client: OpenAIClient, query_type: str,
                                successes: List[Dict], failures: List[Dict]) -> List[Dict]:
    """Generate query-type-specific experiences."""
...
Generate 4-6 QUERY-TYPE-SPECIFIC EXPERIENCES for {query_type} tasks. These should be:
...
[
    {{
        "experience_id": "{query_type[:3]}_001",
        "title": "Short title (3-5 words)",
...
def main():
    ...
    output_path = os.path.expanduser(
        "memory_data/search/claude_style_experiences_search.json"
    )
    ...
    print("\n=== Generating Query-Type-Specific Experiences ===")
    query_type_experiences = {}
    for query_type, data in categorized.items():
        print(f"  Processing {query_type}...")
        experiences = generate_query_type_experiences(
            client, query_type,
            data['success'], data['failure']
        )
        query_type_experiences[query_type] = experiences
    ...
    output = {
        ...
        "query_type_experiences": query_type_experiences,
    ...
    print(f"Query-type-specific experiences: {sum(len(s) for s in query_type_experiences.values())}")
