"""
Generate Claude-style experiences for WebShop agent using o3 API.
...
Generate 10-15 GENERAL EXPERIENCES that apply across ALL product types in web shopping. These should be:
...
def generate_task_specific_experiences(client: OpenAIClient, product_type: str,
                                   successes: List[Dict], failures: List[Dict]) -> List[Dict]:
    """Generate task-specific experiences for a particular product type."""
...
Generate 4-6 TASK-SPECIFIC EXPERIENCES for shopping {product_type.replace('_', ' ')} products. These should be:
...
[
    {{
        "experience_id": "{product_type[:3]}_001",
        "title": "Short title (3-5 words)",
...
def main():
    ...
    output_path = os.path.expanduser(
        "memory_data/webshop/claude_style_experiences.json"
    )
    ...
    for product_type, data in categorized.items():
        if data['success'] or data['failure']:  # Only process if we have data
            ...
            task_specific_experiences[product_type] = experiences
