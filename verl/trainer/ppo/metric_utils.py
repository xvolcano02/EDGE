...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    exp_rollout_metrics = {}
    if "experience_injected" in batch.non_tensor_batch:
        unique_experience_injected = batch.non_tensor_batch["experience_injected"][unique_idx].astype(bool)

        if "trajectory_success" in batch.non_tensor_batch:
            unique_trajectory_success = batch.non_tensor_batch["trajectory_success"][unique_idx].astype(np.float32)
            experience_success = unique_trajectory_success[unique_experience_injected]
            w_o_experience_success = unique_trajectory_success[np.logical_not(unique_experience_injected)]
        else:
            experience_success = np.array([])
            w_o_experience_success = np.array([])

        response_length_np = response_length.detach().cpu().numpy().astype(np.float32)
        traj_uids = batch.non_tensor_batch['traj_uid']
        trajectory_response_lengths = np.array([
            response_length_np[traj_uids == traj_uid].sum()
            for traj_uid in unique_traj_uid
        ], dtype=np.float32)
        experience_response_lengths = trajectory_response_lengths[unique_experience_injected]
        w_o_experience_response_lengths = trajectory_response_lengths[np.logical_not(unique_experience_injected)]

        if len(experience_response_lengths) > 0 and len(w_o_experience_response_lengths) > 0:
            exp_rollout_metrics.update({
                "exp_rollout/experience_trajectory_response_length": float(np.mean(experience_response_lengths)),
                "exp_rollout/w_o_experience_trajectory_response_length": float(np.mean(w_o_experience_response_lengths)),
            })

        if len(experience_success) > 0 and len(w_o_experience_success) > 0:
            experience_mean_success_rate = float(np.mean(experience_success))
            w_o_experience_mean_success_rate = float(np.mean(w_o_experience_success))
            exp_rollout_metrics.update({
                "exp_rollout/experience_mean_success_rate": experience_mean_success_rate,
                "exp_rollout/w_o_experience_mean_success_rate": w_o_experience_mean_success_rate,
                "exp_rollout/experience_gain_diff_mean": experience_mean_success_rate - w_o_experience_mean_success_rate,
            })
